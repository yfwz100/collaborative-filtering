...
class UserBasedCF:
    ...
    def userSimilarity(self,train = None):
        ...
        for u in train.keys():
            for v in train.keys():
                ...
                self.userSim[u][v] = sum(1/(10**(-1/log(self.k(m)))) for m in set(train[u].keys()) & set(train[v].keys()))
    ...
    def recommend(self,user,train = None,k = 8,nitem = 40):
            ...
            for v,wuv in sorted(self.userSimBest[user].items(),key = lambda x:x[1],reverse = True)[0:k]:
                for i in train[v].keys():#ignore the rui value
